...
export const MovieDetails = ({
  ...
  const getSimilarMoviesHandler = async () => {
    try {
      ...
      if (response.success) {
        console.log(response.similarMovies);
  ...
  return (
    <div className="bg-gradient-to-b from-[#110c25]/90 
    via-[#0b0818]/95 to-[#0b0818] text-white">
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
                  ...
                  <div>
                    ...
                    <div className="flex space-x-3 sm:space-x-4 overflow-x-auto pb-2">
                      {streamingServices.map((service, index) => (
                        ...
                        >
                          <div
                            className={`w-10 h-10 sm:w-12 sm:h-12 ${service.color} rounded-full flex items-center justify-center mb-2 relative`}
                          >
                            {service.isIcon ? (
                              <i
                                className={`${service.icon} text-white text-sm sm:text-base`}
                              ></i>
                            ) : (
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                        ...
                        <p className="bg-yellow-500 font-bold text-black px-2 py-1 rounded text-sm sm:text-lg">
                          {movieData["data"].Rated}
                        </p>
                      </div>

                      <p className="text-gray-400 text-sm sm:text-base mt-2">
                        {movieData["data"].Released} • {movieData["data"].genre}{" "}
                ...
                <div className="text-right">
                  <p className="text-gray-400 text-sm sm:text-base">
                    ...
                    <select
                      ...
                      className="text-white bg-transparent ml-2 border-none outline-none text-sm sm:text-base"
                    >
                ...
                <div>
                  ...
                  <div className="flex space-x-3 sm:space-x-4 overflow-x-auto pb-2">
                    {actions.map((action) => (
                      <button
                        key={action.id}
                        className="flex flex-col items-center space-y-2 text-gray-400 hover:text-white transition-colors flex-shrink-0"
                      >
                        <div
                          className={`w-10 h-10 sm:w-12 sm:h-12 ${action.color} rounded-full flex items-center justify-center transition-colors`}
                        >
                          <i
                            className={`${action.icon} text-sm sm:text-base`}
                          ></i>
                        </div>
        ...
        <div className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-y pb-6 border-gray-700">
          <h3 className="text-lg sm:text-xl font-semibold mb-4">Synopsis</h3>
          <p className="text-gray-400 leading-relaxed max-w-4xl text-sm sm:text-base">
            {movieData["data"].storyLine}
          </p>
        </div>
        {similarLoading && (
          <div className="flex justify-center mt-9 items-center mb-8">
        ...
        {!similarLoading && (
          <div className="flex gap-4 justify-center mt-9">
            {SimilarMovies.map(
              (movie: SimilarMoviesResponse["similarMovies"][0]) => {
                return <SimilarMoviesComponent movie={movie["data"]} />;
              }
            )}
          </div>
        )}
